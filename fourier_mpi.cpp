...
// Init origin function (oR, oI) and fR, fI for computations [Given]
...
void printFT(double reals[], double imag[], int length) {
  for (int k = 0; k < length; k++) {
    cout << k << " " << reals[k] << " " << imag[k] << endl;
  }
}

// manager's subroutine
void manager(int N, int numTasks, double oR[], double oI[], double fR[], double fI[]) {
  ...
  int k = 0, valuesRecieved = 0, rankExpected = 1;
  double tempR, tempI;
  for (int i = 0; i < numTasks - 1; i++) {
    MPI_Send(&i, 1, MPI_INT, i + 1, 0, MPI_COMM_WORLD);
  }
  k = numTasks - 1;
  while (valuesRecieved < N) {
    MPI_Status status;  // expecting k value calculated within status
    MPI_Recv(&tempR, 1, MPI_DOUBLE, rankExpected, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    MPI_Recv(&tempI, 1, MPI_DOUBLE, rankExpected, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    int rank = status.MPI_SOURCE, tag = status.MPI_TAG;  // make sure works??
    rankExpected++;
    if (rankExpected == numTasks)
      rankExpected = 1;  // too high, set back to first worker
  ...
  // UNCOMMENT FOR SUBMISSION (THIS FOR EASE OF GETTING TIMINGS)
  printFT(fR, fI, N);
  delete [] fR;
  delete [] fI;
}  // END manager's routine

// all workers' subroutine
void worker(int N) {
...
// preprocess command args to ensure they're good
